...
async function fetchQueuedCards(force=false){return fetchCardsByIds(reviewIds(),{force});}
...
async function removeFlag(id){saveIds(ids().filter(x=>x!==Number(id)));activeReviewIds=activeReviewIds.filter(x=>x!==Number(id));suggestionsCache.delete(Number(id));await renderReviewFresh();}
