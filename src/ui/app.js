...
function renderListeningPractice(){
 ...
 const revealForAudioIssue=issue=>{usedTextFallback=true;audioIssue=issue;document.getElementById('practice-visible-text').innerHTML=`<p class="prompt" lang="nl">${esc(audioText)}</p><p class="small muted">Marked ${issue}. Choose the meaning below, then Check answer to continue. This counts only as recognition.</p>`;document.getElementById('practice-audio-unclear').disabled=true;document.getElementById('practice-audio-unavailable').disabled=true;};
