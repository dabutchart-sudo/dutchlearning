...
function proofReport(){const r=state.lastProof;if(!r)return '';const retakeReady=r.type==='mastery'&&!r.passed&&phase(state.progress[r.concept],dayKey(now()))==='proof-ready';return `<article class="card evidence-card proof-report"><div class="eyebrow">Latest ${esc(r.type)} result · ${esc(r.concept)}</div><h2>${r.passed?'Grammar proven in both directions':r.type==='mastery'?retakeReady?'Mastery retake ready':'Mastery retake opens next study day':'More practice before the next test'}</h2><p>NL → EN: ${r.directions['nl-en'].correct}/${r.directions['nl-en'].total} · EN → NL: ${r.directions['en-nl'].correct}/${r.directions['en-nl'].total}</p><p class="muted small">${r.passed?(r.type==='mastery'?'Retention opens '+esc(state.progress[r.concept].retentionDue)+'.':'The next concept is unlocked. This one will return for maintenance.'):r.type==='mastery'?retakeReady?'Start the retake before ordinary practice so all 20 questions are available.':'You can practise today. The retake opens on your next study day.':'Complete eight successful practice questions before trying fresh proof material again.'}</p></article>`}
...
function proofAction(id,offer=dailyProofOffer(state,content,now())){const p=state.progress[id],ph=phase(p,dayKey(now()));
 ...
 return `<div class="rule"><strong>${type==='mastery'?'Ready for mastery proof':'Ready to check retention'}</strong><p class="small">${type==='mastery'?'10 unseen Dutch → English and 10 unseen typed English → Dutch. Pass with 19/20 grammar overall and at least 9/10 in each direction.':'5 unseen Dutch → English and 5 unseen typed English → Dutch. Retention still requires 10/10 grammar.'} No word help. The test uses this day’s questions, so start it before ordinary practice if you want it today.</p>${hideButton?'':button('proof',type==='mastery'?'Start Mastery Test':'Start Retention Test',true,!!reason)}${!hideButton&&reason?`<p class="small muted">${esc(reason)}</p>`:''}</div>`;
 }
 ...
 return `<p class="muted small">${p.masteredAt?'Retained and in maintenance.':failedMastery?'Your mastery retake opens on your next study day. You can practise today.':`Proof opens after 40 practice attempts. Independent production is tracked separately.${p.remedial?' '+p.remedial+' successful remedial questions remain.':''}`}</p>`;
}
