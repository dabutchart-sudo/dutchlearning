...
import {dutchVoice,speak,prepareSpeech,canUseServerListen} from './speech.js';
...
function renderListeningPractice(){
 ...
 const audioText=q.audio,correctMeaning=q.answer;let raw='',usedTextFallback=false,locked=false;
 ...
 on('practice-text-fallback',()=>{usedTextFallback=true;document.getElementById('practice-visible-text').innerHTML=`<p class="prompt" lang="nl">${esc(audioText)}</p><p class="small muted">Text shown: this answer will count only as recognition within this temporary session.</p>`;document.getElementById('practice-text-fallback').disabled=true;});
 on('check-listening-practice',()=>{if(locked)return;if(!raw){notify('Choose an answer first.');return;}locked=true;const next=answerListeningPractice(listeningSession,raw,{usedTextFallback});const result=next.answers.at(-1);listeningSession=next;const upcoming=currentListeningQuestion(listeningSession);if(upcoming)prepareSpeech(upcoming.audio);el.querySelectorAll('button').forEach(button=>button.disabled=true);document.getElementById('feedback').innerHTML=`<div class="feedback ${result.correct?'ok':'bad'}"><strong>${result.correct?'Meaning understood':'Not this time'}</strong><span class="correct" lang="nl">${esc(audioText)}</span><span class="meaning">${esc(correctMeaning)}</span><div class="badges"><span class="badge">${usedTextFallback?'Recognition only':'Listening diagnostic'}</span><span class="badge">Does not change progress</span></div></div>`;document.querySelector('.actions').innerHTML=button('next-listening-practice',upcoming?'Continue':'View listening summary');on('next-listening-practice',renderListeningPractice);document.getElementById('next-listening-practice').focus();});
