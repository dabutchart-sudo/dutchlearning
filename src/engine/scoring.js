...
export const labels={spelling:'Spelling',word_order:'Word order',verb_form:'Verb form',article:'Article',subject:'Subject',vocabulary:'Vocabulary',missing_word:'Missing word',extra_word:'Extra word',translation:'Meaning',negation:'Negation',assistance:'Word recall'};
...
export function assess(q,raw,{assisted=false,knownWords=new Set()}={}){
 ...
 for(let i=0;i<expected.length;i++){
  ...
  if(verbSlot&&(q.forms.includes(b)||a.endsWith('en')&&!b.endsWith('en')||a.startsWith('ge')&&!b.startsWith('ge')||b===a+'t'||a===b+'t'||/[dt]$/.test(a)&&a.slice(0,-1)===b.slice(0,-1)))return result(false,true,'verb_form');
  if(['de','het','een'].includes(a)||['de','het','een'].includes(b))return result(false,true,'article');
  if(['geen','niet'].includes(a)||['geen','niet'].includes(b))return result(false,true,'negation');
  if(structural.has(a)||structural.has(b))return result(false,true,verbSlot?'verb_form':'subject');
  // A one-letter unknown typo can demonstrate structure, but NEVER a known word or changed verb ending.
  if(a.length>=4&&distance(a,b)===1&&!knownWords.has(b)&&(!verbSlot||a.slice(-1)===b.slice(-1))){spelling=true;continue;}
  return result(null,false,'vocabulary');
 }
 return result(true,!spelling,spelling?'spelling':null);
}
