...
import {visualSuitabilityForRecord} from './visual-suitability.js';
...
export function visualNeedForRecord(record={},state={}, {today,days=VISUAL_NEED_WINDOW_DAYS,minRecallDays=VISUAL_NEED_RECALL_DAYS}={}){
 ...
 if(visualCueForRecord(record))return {needed:false,reason:'image-available',recallMisses:0,recallMissDays:0,latestError:null,requiresSuitabilityCheck:false,suitability:visualSuitabilityForRecord(record)};
 ...
 const suitability=visualSuitabilityForRecord(record);
 return {needed,reason:needed?'repeated-recall':latest?.correct===true?'recovered':missDays.size?'not-enough-days':'no-recall-evidence',recallMisses:misses.length,recallMissDays:missDays.size,latestError,requiresSuitabilityCheck:needed&&suitability.status==='review',suitability};
}

export function visualNeedSummary(cards=[],state={}, {today,days=VISUAL_NEED_WINDOW_DAYS,minRecallDays=VISUAL_NEED_RECALL_DAYS}={}){
 const records=cards.map(record=>({record,need:visualNeedForRecord(record,state,{today,days,minRecallDays})})),candidates=records.filter(x=>x.need.needed),blocked=candidates.filter(x=>x.need.suitability.status==='blocked'),review=candidates.filter(x=>x.need.suitability.status==='review'),suitable=candidates.filter(x=>x.need.suitability.status==='suitable');
 return {withImages:records.filter(x=>visualCueForRecord(x.record)).length,candidates:candidates.length,candidateRecords:candidates,blocked:blocked.length,blockedRecords:blocked,semanticReview:review.length,semanticReviewRecords:review,generationReady:suitable.length,generationReadyRecords:suitable,days,minRecallDays};
}
