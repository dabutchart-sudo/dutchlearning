...
test('outline uses the full registered curriculum, not just the base content pack',()=>{
 ...
 assert.equal(outline.total,26);assert.equal(outline.current.id,'F1');assert.equal(outline.current.status,'lesson');assert.equal(outline.retained,0);
 assert.equal(outline.topics.find(c=>c.id==='A1.20').status,'upcoming');assert.equal(outline.planned.length,6);
 assert.equal(outline.planned[0].id,'A1.21');assert.equal(outline.planned.at(-1).id,'A1.26');assert.equal(JSON.stringify(s),before);
});
...
test('outline uses configured practice requirements and handles completed course',()=>{
 ...
 const o=courseOutline(s,content,{today});assert.equal(o.current,null);assert.equal(o.retained,26);assert.equal(o.planned.length,6);
});
...
test('empty dashboard shows no fabricated trend or fluency score; known history is read-only',()=>{
 const s=fresh(),before=JSON.stringify(s);const html=coursePage(s,content,{today});
...
test('chart has an accessible numerical alternative and separates evidence groups',()=>{
 ...
 const html=coursePage(s,content,{today});assert.match(html,/course-chart-description/);assert.match(html,/View daily numbers/);assert.match(html,/Every attempt counts here/);assert.match(html,/data-course-cohort="supported"/);
 const supported=coursePage(s,content,{today,cohort:'supported'});assert.match(supported,/kept separate from independent sentence writing/);
});
test('locked topics are browsable but cannot launch lessons; planned topics never become playable',()=>{
 const s=fresh();const html=coursePage(s,content,{today,pane:'syllabus'});assert.match(html,/A1.26/);assert.match(html,/A2 · Beyond the basics/);assert.match(html,/COMING LATER/);assert.doesNotMatch(html,/data-course-concept="A1.21"/);
