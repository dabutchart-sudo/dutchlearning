...
test('heard answers create session-only listening diagnostics',()=>{
 ...
 assert.deepEqual(listeningPracticeSummary(session),{total:1,heard:1,heardCorrect:1,textFallbacks:0,audioUnclear:0,audioUnavailable:0,audioIssues:[],countsTowardProgress:false});
 assert.equal(JSON.stringify(state),before);
});
...
test('revealing text changes evidence to recognition and never claims listening proof',()=>{
 ...
 assert.deepEqual(listeningPracticeSummary(session),{total:1,heard:0,heardCorrect:0,textFallbacks:1,audioUnclear:0,audioUnavailable:0,audioIssues:[],countsTowardProgress:false});
});
...
test('unclear and unavailable audio remain answerable recognition fallbacks',()=>{
 ...
 assert.deepEqual({...summary,audioIssues:undefined},{total:2,heard:0,heardCorrect:0,textFallbacks:2,audioUnclear:1,audioUnavailable:1,audioIssues:undefined,countsTowardProgress:false});
 assert.deepEqual(summary.audioIssues.map(({issue,audio,meaning})=>({issue,audio,meaning})),session.questions.map((q,index)=>({issue:index?'unavailable':'unclear',audio:q.audio,meaning:q.answer})));
});
