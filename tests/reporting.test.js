...
test('report UI stacks daily load, fits the viewport and anchors upcoming counts to bars',()=>{const css=readFileSync(new URL('../src/ui/reporting.css',import.meta.url),'utf8');const ui=readFileSync(new URL('../src/ui/reporting.js',import.meta.url),'utf8');assert.match(css,/report-bar-stack\{[^}]*flex-direction:column-reverse/);assert.match(css,/report-bars\{[^}]*overflow:hidden/);assert.match(css,/report-bar-col\{[^}]*min-width:0/);assert.match(css,/report-bar-value\{[^}]*position:absolute[^}]*top:-16px/);assert.match(ui,/barClass:'upcoming'/);assert.match(ui,/report-bar-value/);});
