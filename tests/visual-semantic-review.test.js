...
import {visualSemanticDecision,visualSemanticReviewQueue,recordVisualSemanticDecision,visualGenerationCandidate,visualSemanticReviewSummary} from '../src/engine/visual-semantic-review.js';
...
const correct=(id,date)=>({cardId:String(id),date,timestamp:`${date}T10:00:00Z`,meaningful:true,correct:true,errorType:'none',expected:`woord${id}`,answer:`woord${id}`});
...
test('summary separates pending, suitable, unsuitable and generation-ready words',()=>{
 const cards=[card(1),card(2),card(3)],state={flashcardProduction:{attempts:[miss(1,'2026-09-08'),miss(1,'2026-09-10'),miss(2,'2026-09-08'),miss(2,'2026-09-10'),miss(3,'2026-09-08'),miss(3,'2026-09-10')]}};
 recordVisualSemanticDecision(state,cards[1],'suitable',{today:'2026-09-11'});
 recordVisualSemanticDecision(state,cards[2],'unsuitable',{today:'2026-09-11'});
 const summary=visualSemanticReviewSummary(cards,state,{today:'2026-09-11'});
 assert.equal(summary.pending,1);
 assert.equal(summary.suitable,1);
 assert.equal(summary.unsuitable,1);
 assert.equal(summary.generationReady,1);
 assert.equal(String(summary.pendingRecords[0].id),'1');
});

test('later successful recall removes a previously suitable word from generation readiness',()=>{
 const record=card(1),state=stateFor(1);
 recordVisualSemanticDecision(state,record,'suitable',{today:'2026-09-10'});
 state.flashcardProduction.attempts.push(correct(1,'2026-09-11'));
 const summary=visualSemanticReviewSummary([record],state,{today:'2026-09-11'});
 assert.equal(summary.suitable,0);
