...
test('production Listen stops earlier audio before the replacement clip downloads',async()=>{const instances=[];class FakeAudio{constructor(){this.src='';this.currentTime=0;this.pauses=0;instances.push(this);}setAttribute(){}pause(){this.pauses++;}play(){return Promise.resolve();}}globalThis.Audio=FakeAudio;globalThis.URL={createObjectURL:()=>'blob:new',revokeObjectURL(){}};globalThis.location={hostname:'dabutchart-sudo.github.io'};globalThis.fetch=async url=>{if(String(url).includes('constants.js'))return {ok:true,text:async()=>`export const SUPABASE_ANON_KEY='anon';`};return new Promise(()=>{});};mockSynth([]);const speech=await import(`${speechUrl.href}?replace=${Date.now()}`);speech.unlockListenAudio();const player=instances[1];player.src='blob:old';player.currentTime=4;speech.speak('Jullie praten.');assert.ok(player.pauses>=1);assert.equal(player.currentTime,0);delete globalThis.speechSynthesis;delete globalThis.SpeechSynthesisUtterance;delete globalThis.Audio;delete globalThis.fetch;delete globalThis.location;});
