...
import {independentRecallCandidates,independentExercise,independentAttemptsToday,independentMissCoolingDown,INDEPENDENT_MISS_COOLDOWN_DAYS} from '../src/engine/independent-recall.js';
...
test('a failed independent word cools down for three study days',()=>{const s=structuredClone(independentState);s.flashcardProduction.attempts.push({cardId:'1',date:'2026-09-11',stage:PRODUCTION_STAGE.INDEPENDENT,meaningful:true,correct:false});assert.equal(INDEPENDENT_MISS_COOLDOWN_DAYS,3);assert.equal(independentMissCoolingDown(s,'1','2026-09-11'),true);assert.equal(independentMissCoolingDown(s,'1','2026-09-12'),true);assert.equal(independentMissCoolingDown(s,'1','2026-09-13'),true);assert.equal(independentMissCoolingDown(s,'1','2026-09-14'),false);});

test('cooling-down words are excluded while other independent words remain eligible',()=>{const s=structuredClone(independentState);s.flashcardProduction.attempts.push({cardId:'1',date:'2026-09-10',stage:PRODUCTION_STAGE.INDEPENDENT,meaningful:true,correct:false});const q=independentRecallCandidates([card(1),card(2)],s,{today:'2026-09-12',random:()=>.5});assert.equal(q.length,1);assert.equal(q[0].id,'2');});
